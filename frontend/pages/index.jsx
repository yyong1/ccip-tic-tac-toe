  ...
  const move = async (row, column) => {
    if (sessionId) {
      ...
      try {
        ...
        console.error(err);
        let reason = err.reason || err.error?.message;
        if (!reason && err.error?.data) {
          reason = decodeRevertReason(err.error.data);
        }
        setText(reason || "Transaction failed");
      }
    }
  };
